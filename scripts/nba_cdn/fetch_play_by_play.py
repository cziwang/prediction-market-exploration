"""Fetch play-by-play from cdn.nba.com and store in S3.

Two modes:
  Default  — fetch PBP for today's games (from scoreboard).
  --season — backfill PBP for all completed games in a season schedule
             stored in S3. Skips games already fetched. Run fetch_schedule
             first to populate the schedule.

Usage:
    python -m scripts.nba_cdn.fetch_play_by_play                   # today's games
    python -m scripts.nba_cdn.fetch_play_by_play --season 2025-26  # backfill season
    python -m scripts.nba_cdn.fetch_play_by_play --season 2025-26 --max-games 10
"""
...
def _game_ids_from_schedule(season: str) -> list[str]:
    ...
    return [
        ...
        if g.get("gameStatus") == 3  # completed games only
    ]
...
def main() -> None:
    ...
    existing_keys = set(list_keys("nba_cdn", "play_by_play"))
    to_fetch = [gid for gid in game_ids if f"nba_cdn/play_by_play/{gid}.json" not in existing_keys]
    ...
    for i, game_id in enumerate(to_fetch, 1):
        ...
        print(f"  [{i}/{len(to_fetch)}] {game_id}: {len(actions)} actions")
        time.sleep(args.throttle)
